...
import { Top10MoviesTrended } from "../top-10-movies-trended/top-10-movies-trended.use-case"

export function makeTop10MoviesTrended() {
  const movie_repo = new MovieSequelizeRepository(MovieModel)
  const movie_api_http = new TmdbApiHttp()
  const use_case = new Top10MoviesTrended(movie_repo, movie_api_http)
  return use_case
}
